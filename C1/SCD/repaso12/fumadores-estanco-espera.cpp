...
public:
   ...
   void esperarIngrediente(int ingrediente);
   void terminar_de_fumar();
};
...
void Estanco::esperarIngrediente(int ingrediente){
   if (mostrador != ingrediente){
      cout << "                  Fumador " << ingrediente << " espera para recoger su ingrediente" << endl << flush;
      esta_mio[ingrediente].wait();
   }
   cout << "                  Fumador " << ingrediente << " recoge su ingrediente" << endl << flush;

}

void Estanco::terminar_de_fumar(){
   mostrador = -1;
...
{
   while (true){
      monitor->esperarIngrediente(indice);
      fumar(indice);
      monitor->terminar_de_fumar();
   }
}
